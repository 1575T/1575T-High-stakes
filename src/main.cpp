...
void ez_template_extras() {
  ...
  if (!pros::competition::is_connected()) {
    ...
    if (master.get_digital(pros::E_CONTROLLER_DIGITAL_LEFT) && master.get_digital(DIGITAL_RIGHT)) {
...
void opcontrol() {
  ...
  while (true) {
    ...
    //Standard split arcade (joystick controls)
    ...
    //controls for mogo clamp
    if(master.get_digital_new_press(DIGITAL_L1)){
      mogo_clamp.toggle();
    }

    //controls for the doinker
